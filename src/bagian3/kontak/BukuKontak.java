...
public class BukuKontak {
    ...
    public void muatDariBerkas() {
        ...
        try (BufferedReader pembaca = new BufferedReader(new FileReader(namaBerkas))) {
            ...
            while ((baris = pembaca.readLine()) != null) {
                ...
                if (bagian.length == 3) { // Menggunakan 3 bagian sesuai soal nomor 2 (nama;nomor;email)
    ...
    public void hapusKontak(String nama) {
        ...
        // Menggunakan perulangan index untuk menghindari ConcurrentModificationException
        for (int i = 0; i < daftar.size(); i++) {
            if (daftar.get(i).getNama().equalsIgnoreCase(nama)) {
                daftar.remove(i); // Menghapus kontak dari ArrayList
                ditemukan = true;
                System.out.println("Kontak dengan nama '" + nama + "' berhasil dihapus dari daftar.");
                break; // Berhenti setelah menemukan dan menghapus data pertama yang cocok
            }
        }

        // Jika berhasil dihapus, langsung simpan perubahan ke berkas
        if (ditemukan) {
            simpanKeBerkas(); 
        } else {
